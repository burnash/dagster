...
def test_proxying_to_dagster(mock_airflow_variable: None) -> None:
    ...
    assert isinstance(
        globals_fake["task_is_proxied"].task_dict["task"], BaseProxyToDagsterOperator
    )
    ...
    assert (
        "1 Task Marked as Proxied to Dagster" in original_globals["initially_not_proxied"].tags
    )
