...
class PipesECSClient(PipesClient, TreatAsResourceParam):
    ...
    def run(
        ...
        *,
        ...
        with open_pipes_session(
            ...
            def get_cloudwatch_params(container_name: str) -> Optional[Dict[str, str]]:
                ...
                if log_config := log_configurations.get(container_name):
                    if log_config["logDriver"] == "awslogs":
                        ...
                        context.log.warning(
                            f"[pipes] Unsupported log driver {log_config['logDriver']} for Pipes container {container_name} in task {task['taskArn']}. Dagster Pipes won't be able to read CloudWatch logs from this container."
                        )
                else:
                    context.log.warning(
            ...
            try:
                if (
                    ...
                    and len(task["containers"]) > 1  # pyright: ignore (reportTypedDictNotRequiredAccess)
                    ...
                    pipes_container_name = task["containers"][0]["name"]  # pyright: ignore (reportTypedDictNotRequiredAccess)
        ...
        context.log.info(f"[pipes] ECS task {task['taskArn']} completed")  # pyright: ignore (reportGeneralTypeIssues)
