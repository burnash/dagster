...
export class GraphLayout {
  ...
  layout(rootNodeId?: string): Rect {
    ...
    // Get layout graph.
    const layoutGraph = getLayoutGraph(rootNode?.id || '', nodes, this.modelGraph, this.testMode);
...
export function getNodeWidth(node: ModelNode, modelGraph: ModelGraph, testMode = false) {
...
export function getNodeHeight(node: ModelNode, modelGraph: ModelGraph, testMode = false) {
  if (testMode) {
...
export function getLayoutGraph(
  ...
  for (const node of nodes) {
    ...
    const dagreNode: DagreNode = {
      id: node.id,
      width: node.width || (useFakeNodeSize ? 10 : getNodeWidth(node, modelGraph, testMode)),
      height: useFakeNodeSize ? 10 : getNodeHeight(node, modelGraph, testMode),
    };
